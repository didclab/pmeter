...
def begin_measuring(user, folder_path, file_name, folder_name,interface='', measure_tcp=True, measure_udp=True, measure_kernel=True, measure_network=True, print_to_std_out=False, interval=1, latency_host="google.com", measurement=1, length="0s"):
    metric = ODS_Metrics()
...
if __name__ == '__main__':
    ...
    if arguments['measure']:
        ...
        print("Parameters recieved: \n", "interface=",interface, "measure network=", network_only, "measure kernel=", kernel_only, "interval between measurements=", pause_between_measure, "times to measure default is 1=", times_to_measure, 'file_name=', file_name, 'folder_name=', folder_name, 'folder_path=', folder_path)
        begin_measuring(folder_path=folder_path, folder_name=folder_name, file_name=file_name,user=user, interface=interface,measure_tcp=tcp_only, measure_kernel=kernel_only, measure_network=network_only, measure_udp=udp_only, print_to_std_out=std_out_print, interval=pause_between_measure, length=lengthOfExperiment, measurement=int(times_to_measure))
